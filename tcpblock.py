...
class Datablock(object):
    ...
    def readall(self):
        ...
        while not ready[0]:
            ...
            if not ready[0]:
                self.error_stream("In readall(): not ready, select returns '%s'"%(str(ready)))
        ...
        while len(self._recv)!=self.read_size:#while rem>0:
            ...
            if len(self._recv) == 0:
                ...
                #    return False
                self.debug_stream("Nothing received from the PLC (try %d)"%(retries))
                time.sleep(0.1)
            else:
                retries = 0
                if len(self._recv) > self.read_size:
                    ...
                    self.debug_stream(">> Received 1 DB (%d)"%(len(self._recv)))
                ...
                if self.isGoodBlock(self._recv):
                    ...
                    self.debug_stream("< discard data (%d)"%(len(self._recv)))
                    self._recv = ''
...
#            print("get Exception a=%s"%(repr(a)))
    ...
    def rewrite(self):
        ...
        try:
            assert (len(write_str)==self.write_size), 'len(write_str) %d != Write Size %d' % (len(write_str),self.write_size)
        except Exception,e:
...
def open_datablock(hostname, port, read_size, write_size, REMOTE='0.0.0.0',
                     info=None,debug=None,warn=None,error=None,checkAddr=None):
    ...
    if info:
        info("open_datablock(hostname:'%s',port:%d,read_size:%d,write_size:"\
              "%d,REMOTE:'%s')"%(hostname, port, read_size, write_size, REMOTE))
    import os
    ...
    if conn is None:
        try:
            ...
            if error: error("open_datablock() exception: %s"%(e))
            raise e
    db = Datablock(sock, read_size, write_size,info,debug,warn,error,checkAddr)
    return db
