...
class Interface(object):
    ...
    def __init__(self, sock):
        ...
        self.ptr = lib.daveNewInterface(fds, self.nname, self.localMPI, self.protocol, self.speed)
        if not self.ptr:
...
class Connection(object):
    ...
    def __init__(self, di):
        # stores a reference to di object to ensure that the connection is collected
        # before the interface is collected
        self.interface = di
        mpi = c_int(2)
        rack = c_int(0)
        slot = c_int(2)
    ...
    def read_bytes(self, what, dbnum, offset, nwords, buf=None):
        ...
        res = lib.daveReadBytes(self.ptr, what, c_int(dbnum), c_int(offset), c_int(nwords), vp)
        if res:
            msg = 'reading %d words from DB %d at offset %d failed (%s)' % (nwords, dbnum, offset, res)
    ...
    def write_bytes(self, what, dbnum, offset, nwords, buf):
        ...
        res = lib.daveWriteBytes(self.ptr, what, c_int(dbnum), c_int(offset), c_int(nwords), buf)
        if res:
            msg = 'writing %d words into DB %d at offset %d failed (%s)' % (nwords, dbnum, offset, res)
            raise DaveException(msg)
...
class Datablock(object):
    ...
    def readall(self):
        self.dc.read_bytes(self.what, self.dbnum, self.offset, self.nbytes, self.buf)
...
def open_isotcp_datablock(hostname, dbnum, nwords, offset=0, what=DAVE_DB):
    '''Creates a Datablock for communicating with host 'host' on databloc dbnum which has size 'size'.
    '''
